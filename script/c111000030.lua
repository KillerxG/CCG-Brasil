...
function s.initial_effect(c)
    ...
    e10:SetOperation(s.hdop)
    c:RegisterEffect(e10)

    -- Efeito 3 (e11): "Yoko, the Graceful Mayakashi" que você controla é inafetada por efeitos de cartas
    local e11 = Effect.CreateEffect(c)
    e11:SetType(EFFECT_TYPE_FIELD)
    e11:SetCode(EFFECT_IMMUNE_EFFECT)
    e11:SetRange(LOCATION_FZONE)
    e11:SetTargetRange(LOCATION_MZONE, 0)
    e11:SetTarget(aux.TargetBoolFunction(Card.IsCode, 3486020))
    e11:SetValue(s.efilter)
    c:RegisterEffect(e11)

    -- Efeito 4 (e12): "Yoko, the Graceful Mayakashi" pode atacar diretamente
    local e12 = Effect.CreateEffect(c)
    e12:SetType(EFFECT_TYPE_FIELD)
    e12:SetCode(EFFECT_DIRECT_ATTACK)
    e12:SetRange(LOCATION_FZONE)
    e12:SetTargetRange(LOCATION_MZONE, 0)
    e12:SetTarget(aux.TargetBoolFunction(Card.IsCode, 3486020))
...
function s.tkop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		local token=Duel.CreateToken(tp,3486020)
...
function s.indct(e, re, r, rp)
    -- Retorna 1 carga de proteção contra destruições cuja razão (r) seja batalha (REASON_BATTLE)
...
-- ==========================================================
-- Efeito (e10): Descarte Aleatório e Banir Extra Deck
-- ==========================================================
...
function s.hdtg(e, tp, eg, ep, ev, re, r, rp, chk)
    -- Verifica passivamente se o oponente tem cartas na mão
...
function s.hdop(e, tp, eg, ep, ev, re, r, rp)
    ...
    -- Seleciona e descarta 1 carta aleatória
    local cg = hg:RandomSelect(tp, 1)
    if Duel.SendtoGrave(cg, REASON_EFFECT | REASON_DISCARD) > 0 then
        local tc = cg:GetFirst()
        -- Verifica se a carta chegou no GY e se possui a tipologia Monstro
...
-- ==========================================================
-- Efeito (e11): Imunidade para a Yoko
-- ==========================================================
