...
function s.spfilter(c,e,tp)
  return c:IsSetCard(0x998) and c:IsLevel(4) and c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.xyzfilter1(c,e,tp)
  return c:IsFaceup() and c:IsSetCard(0x998) and c:GetLevel()>0 and Duel.GetLocationCountFromEx(tp,tp,c)>0 
...
function s.xyzfilter2(c,e,tp,mc)
  ...
  return mc:IsType(TYPE_MONSTER,c,SUMMON_TYPE_XYZ,tp) and c:IsSetCard(0x998) and c:IsRankBelow(4) 
