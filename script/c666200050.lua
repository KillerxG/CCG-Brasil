...
function s.eqfilter(c)
	return c:IsFaceup() and c:IsLinkMonster() and ((c:IsRace(RACE_CYBERSE) and (c:IsAttribute(ATTRIBUTE_EARTH) or c:IsAttribute(ATTRIBUTE_WIND))) or c:IsSetCard(0x352))
end
function s.eqval(ec,c,tp)
	return ec:IsControler(tp) and ec:IsFaceup() and ec:IsLinkMonster() and ((ec:IsRace(RACE_CYBERSE) and (ec:IsAttribute(ATTRIBUTE_EARTH) or ec:IsAttribute(ATTRIBUTE_WIND))) or ec:IsSetCard(0x352)) and not c:IsForbidden()
end
...
function s.thfilter1(c)
	return c:IsSetCard(0x352) and c:IsTrap() and c:IsAbleToHand()
end
