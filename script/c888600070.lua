...
function s.initial_effect(c)
	...
	Fusion.AddProcMixRep(c,true,true,aux.FilterBoolFunctionEx(Card.IsType,TYPE_SPIRIT),1,99,s.mfilter)
	--(1)Monster Effects
	--(1.1)Spirit Return
...
function s.mfilter(c,fc,sumtype,tp)
	return c:IsSetCard(0x401,fc,sumtype,tp)
end
--(1.1)Spirit Return
...
function s.rescon(sg,e,tp,mg)
	return sg:GetClassCount(Card.GetCode)==#sg and Duel.IsExistingTarget(Card.IsAbleToHand,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,#sg,nil)
end
function s.c2filter(c)
	return c:IsType(TYPE_SPIRIT) and not c:IsPublic()
end
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsOnField() and Card.IsAbleToHand(chkc) end
	local g=Duel.GetMatchingGroup(s.c2filter,tp,LOCATION_HAND,0,nil)
	if chk==0 then 
		return aux.SelectUnselectGroup(g,e,tp,1,99,s.rescon,0)
	end
	local sg=aux.SelectUnselectGroup(g,e,tp,1,99,s.rescon,1,tp,HINTMSG_CONFIRM)
	Duel.ConfirmCards(1-tp,sg)
	Duel.ShuffleHand(tp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local rg=Duel.SelectTarget(tp,Card.IsAbleToHand,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,#sg,#sg,nil)
