...
function s.initial_effect(c)
	...
	c:RegisterEffect(e5)
	--(2.3)Special Summon "Yokai" from banishment
...
function s.filter(c)
	return c:IsSetCard(0x401b) and c:IsAbleToHand()
end
...
--(2.3)Special Summon "Yokai" from banishment
function s.filspter(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0x401) and c:IsMonster() and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.nstg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and c:IsAbleToHand()
		and Duel.IsExistingMatchingCard(s.filspter,tp,LOCATION_REMOVED,0,1,nil,e,tp) end
...
function s.nsop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.filspter,tp,LOCATION_REMOVED,0,1,1,nil,e,tp)
	if #g>0 then
