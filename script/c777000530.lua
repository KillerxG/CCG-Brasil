...
function s.initial_effect(c)
    ...
    c:RegisterEffect(e1)
	--(2)Search "Rank-Up-Magic" then can banish
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
    ...
    if c:IsRelateToEffect(e) and Duel.SpecialSummon(c, 0, tp, tp, false, false, POS_FACEUP_DEFENSE) > 0 then
        -- "then you can change this card's Level to 4"
...
function s.rmfilter(c)
    return c:IsRace(RACE_DRAGON) and c:IsAttribute(ATTRIBUTE_FIRE) and c:IsAbleToRemove()
end
...
function s.thop(e, tp, eg, ep, ev, re, r, rp)
    ...
    if #g > 0 and Duel.SendtoHand(g, nil, REASON_EFFECT) > 0 and g:GetFirst():IsLocation(LOCATION_HAND) then
        ...
        local rg = Duel.GetMatchingGroup(s.rmfilter, tp, LOCATION_HAND + LOCATION_DECK, 0, nil)
        if #rg > 0 and Duel.SelectYesNo(tp, aux.Stringid(id, 4)) then
            Duel.BreakEffect()
