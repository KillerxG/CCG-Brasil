...
function s.initial_effect(c)	
	...
	c:RegisterEffect(e4)
	--(5)Manipulate Fusion Summon for "Gem-Knight"
...
--(5)Manipulate Fusion Summon for "Gem-Knight"
function s.gemfilter(c,e,tp)
	return c:IsSetCard(SET_GEM_KNIGHT) and c:IsType(TYPE_FUSION) and c:GetLevel()<=8 and Duel.GetLocationCountFromEx(tp,tp,nil,c)>0
		and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,true,true) 
end
function s.gemsp(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.gemfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp) and Duel.IsExistingMatchingCard(Card.IsMonster,tp,LOCATION_HAND+LOCATION_MZONE,0,2,nil) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_EXTRA)
	Duel.SetChainLimit(function(_e,_ep,_tp) return _tp==_ep end)
end
function s.gemop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local tc=Duel.SelectMatchingCard(tp,Card.IsMonster,tp,LOCATION_HAND+LOCATION_MZONE,0,2,2,nil)
	Duel.SendtoGrave(tc,REASON_EFFECT)
	local g=Duel.SelectMatchingCard(tp,s.gemfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp)
	local tf=g:GetFirst()
