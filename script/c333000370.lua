--Data Paladin Sacrament
--Scripted by KillerxG
local s,id=GetID()
function s.initial_effect(c)
    --(1)Fusion Summon
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_FUSION_SUMMON+CATEGORY_TOHAND+CATEGORY_SEARCH)
...
function s.filter2(c,e,tp,m,f)
	return c:IsType(TYPE_FUSION) and c:IsSetCard(0x265) and (not f or f(c))
		and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,false) and c:CheckFusionMaterial(m,nil,tp)
		and Duel.IsExistingMatchingCard(s.addfilter,tp,0,LOCATION_DECK,1,nil)
end
...
function s.filterchk(c)
	return c:IsSetCard(0x265) and not c:IsHasEffect(id)
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local sg=Duel.SelectMatchingCard(1-tp,s.addfilter,1-tp,LOCATION_DECK,0,1,1,nil)
