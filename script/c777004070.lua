...
function s.initial_effect(c)
	--(1)Special Summon Xyz and Fusion
...
function s.counterfilter(c)
	return not (c:IsSummonLocation(LOCATION_EXTRA) and not c:IsType(TYPE_FUSION|TYPE_XYZ))
end
function s.cfilter1(c,e,tp)
	return c:IsFaceup() and c:IsAbleToGraveAsCost() and not c:IsOriginalType(TYPE_TRAP)
		and Duel.IsExistingMatchingCard(s.cfilter2,tp,LOCATION_MZONE,0,1,c,e,tp,c)
end
function s.cfilter2(c,e,tp,tun)
	if not (c:IsFaceup() and c:IsAbleToGraveAsCost() and not c:IsOriginalType(TYPE_TRAP)) then return false end
...
function s.syncfilter(c,e,tp,mg)
	if not (c:IsFacedown() and c:IsType(TYPE_XYZ)) then return false end
...
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	--Cannot Special Summon from the Extra Deck, except Fusion and Xyz Monsters
	local e1=Effect.CreateEffect(c)
	...
	e1:SetTarget(function(_,c) return c:IsLocation(LOCATION_EXTRA) and not c:IsType(TYPE_FUSION|TYPE_XYZ) end)
	e1:SetReset(RESET_PHASE|PHASE_END)
	Duel.RegisterEffect(e1,tp)
	--Clock Lizard check
	aux.addTempLizardCheck(c,tp,function(_,c) return not c:IsOriginalType(TYPE_FUSION|TYPE_XYZ) end)
end
