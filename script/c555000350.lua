...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	--Special Summon Nattustie Token
...
function s.thfilter(c)
	return c:IsSetCard(0x5553) and c:IsType(TYPE_MONSTER) and not c:IsCode(id) and c:IsAbleToHand()
end
...
--Place 1 "Flower Maiden" in your Spell/Trap Zone
...
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chkc then return chkc:IsLocation(LOCATION_ONFIELD) and c:IsControler(tp) and s.plfilter(chkc,e) end
	if chk==0 then return Duel.IsExistingTarget(s.plfilter,tp,LOCATION_ONFIELD,0,1,nil,e) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,s.plfilter,tp,LOCATION_ONFIELD,0,1,1,nil,e)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,1,0,0)
end
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	if tc and tc:IsRelateToEffect(e) and Duel.Destroy(tc,REASON_EFFECT)>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,555000540,0x5553,TYPES_TOKEN+TYPE_TUNER,700,100,2,RACE_WYRM,ATTRIBUTE_EARTH)
		and Duel.SelectYesNo(tp,aux.Stringid(id,3)) then
		Duel.BreakEffect()
		local token=Duel.CreateToken(tp,555000540)
