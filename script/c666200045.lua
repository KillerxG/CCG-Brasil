...
function s.initial_effect(c)
	...
	c:RegisterEffect(e6)
	aux.AddEREquipLimit(c,nil,s.eq2val,Card.EquipByEffectAndLimitRegister,e6)
	...
	c:RegisterEffect(e7)
	aux.AddEREquipLimit(c,nil,s.eq2val,Card.EquipByEffectAndLimitRegister,e7)
end
--(2)Alternative Link Summon
...
function s.eqfilter(c)
	return c:IsFaceup() and c:IsLinkMonster() and ((c:IsRace(RACE_CYBERSE) and (c:IsAttribute(ATTRIBUTE_DARK) or c:IsAttribute(ATTRIBUTE_LIGHT))) or c:IsSetCard(0x352))
end
function s.eqval(ec,c,tp)
	return ec:IsControler(tp) and ec:IsFaceup() and ec:IsLinkMonster() and ((ec:IsRace(RACE_CYBERSE) and (ec:IsAttribute(ATTRIBUTE_DARK) or ec:IsAttribute(ATTRIBUTE_LIGHT))) or ec:IsSetCard(0x352)) and not c:IsForbidden()
end
...
function s.eq2val(ec,c,tp)
	return ec:IsControler(tp) and ec:IsMonster() 
end
function s.eq2filter(c)
	return c:IsMonster() and not c:IsForbidden()
end
function s.thtg1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE|LOCATION_MZONE|LOCATION_REMOVED) and s.eq2filter(chkc) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(s.eq2filter,tp,LOCATION_GRAVE|LOCATION_MZONE|LOCATION_REMOVED,LOCATION_GRAVE|LOCATION_MZONE|LOCATION_REMOVED,1,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	local g=Duel.SelectTarget(tp,s.eq2filter,tp,LOCATION_GRAVE|LOCATION_MZONE|LOCATION_REMOVED,LOCATION_GRAVE|LOCATION_MZONE|LOCATION_REMOVED,1,1,e:GetHandler())
