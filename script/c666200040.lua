...
function s.initial_effect(c)
	...
	e2:SetCondition(function(e) return Duel.IsExistingMatchingCard(s.thcostfilter,e:GetHandlerPlayer(),LOCATION_REMOVED+LOCATION_GRAVE,0,3,nil,e:GetHandlerPlayer()) end)
	c:RegisterEffect(e2)
...
function s.efilter(c,tp)
	return c:IsFaceup() and c:IsRace(RACE_CYBERSE) and Duel.IsExistingMatchingCard(s.eqfilter,tp,LOCATION_REMOVED+LOCATION_GRAVE,0,1,nil,c)
end
...
function s.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.eqfilter,tp,LOCATION_REMOVED+LOCATION_GRAVE,0,1,1,nil,tc)
	local eq=g:GetFirst()
	if eq then
