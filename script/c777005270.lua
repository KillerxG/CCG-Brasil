...
function s.initial_effect(c)	
	--(1)Special Summon Xyz and Synchro
...
function s.counterfilter(c)
	return not (c:IsSummonLocation(LOCATION_EXTRA) and not c:IsType(TYPE_XYZ|TYPE_SYNCHRO))
end
...
function s.cfilter2(c,e,tp,tun)
	...
	local chk=Duel.GetLocationCountFromEx(tp,tp,g,TYPE_XYZ)>=2
...
function s.fusfilter(c,e,tp,mg)
	if not (c:IsFacedown() and c:IsType(TYPE_XYZ) and (not c.material_location or (c.material_location&LOCATION_GRAVE)>0)) then return false end
	return c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:IsXyzSummonable(nil,mg)
end
...
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	--Cannot Special Summon from the Extra Deck, except Xyz and Synchro Monsters
	local e1=Effect.CreateEffect(c)
	...
	e1:SetTarget(function(_,c) return c:IsLocation(LOCATION_EXTRA) and not c:IsType(TYPE_XYZ|TYPE_SYNCHRO) end)
	e1:SetReset(RESET_PHASE|PHASE_END)
	Duel.RegisterEffect(e1,tp)
	--Clock Lizard check
	aux.addTempLizardCheck(c,tp,function(_,c) return not c:IsOriginalType(TYPE_XYZ|TYPE_SYNCHRO) end)
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if #mg~=2 or Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT) or Duel.GetLocationCountFromEx(tp,tp,mg,TYPE_XYZ)<2 then return end
