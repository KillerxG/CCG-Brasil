...
function s.initial_effect(c)
	...
	c:RegisterEffect(e5)
	--(2.3)Set 1 "Heian" Spell
	local e6=Effect.CreateEffect(c)
	e6:SetDescription(aux.Stringid(id,5))
	e6:SetCategory(CATEGORY_TOHAND+CATEGORY_SET)
...
function s.filter(c)
	return c:IsSetCard(0x401) and c:IsAbleToHand() and not c:IsCode(id)
end
...
function s.setfilter(c)
	return c:IsSetCard(0x401b) and c:IsSSetable() and c:IsFaceup()
end
function s.settg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingMatchingCard(s.setfilter,tp,LOCATION_REMOVED,0,1,nil)
...
function s.setop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SSet(tp,g)>0 and c:IsAbleToHand() then
