--Phantom Gunners Preparation
--Scripted by KillerxG
local s,id=GetID()
function s.initial_effect(c)
	--(1)Add 1 "Phantom Gunners" monster
...
function s.thfilter(c)
	return c:IsSetCard(0x302) and c:IsMonster() and c:IsAbleToHand()
end
...
function s.efilter(c,tp)
	return c:IsFaceup() and c:IsSetCard(0x302) and c:IsMonster()
end
function s.eqfilter(c)
	return not c:IsForbidden() and c:IsSetCard(0x302) and c:IsMonster()
end
